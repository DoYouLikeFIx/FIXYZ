...
Use `LIVE_GITHUB_TOKEN` with `npm run capture:supply-chain:evidence` from the workspace root to collect live Dependabot and branch-protection evidence for `FIXYZ`, `BE`, `FE`, and `MOB` in one pass.
