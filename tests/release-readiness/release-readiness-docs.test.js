...
test("full-stack smoke rehearsal runbook links canonical Story 10.4 automation and rollback flow", () => {
  ...
  assert.match(runbook, /run-edge-gateway-validation\.sh/);
...
test("go-no-go checklist template keeps Story 10.4 evidence and blocking rules explicit", () => {
  ...
  assert.match(checklist, /edge-summary\.json/);
  ...
  assert.match(checklist, /Missing Story `10\.4` evidence keeps the release at `no-go`\./);
  assert.match(checklist, /Release readiness remains `no-go` when smoke, edge validation, session isolation, or rollback rehearsal is not `passed`\./);
});
