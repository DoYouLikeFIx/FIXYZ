...
국내 증권사 관점에서, 고객이 HTS·MTS로 수행하는 매수/매도 주문은 FIX 4.2 기반으로 “주전산(핵심) 시스템”을 통해 처리되고, 필요 시 외부 공동망(예: 결제원, 중앙은행 등)을 거쳐 타 기관과 상호 연동된다는 식의 큰 흐름이 교육 자료에서 설명된다. 또한 정보계·대외계 같은 용어도 “고객·계좌 관련 정보 활용(정보계)”과 “대외 공동망 연계(대외계)”라는 취지로 정리된다. citeturn9view0turn9view1

다만 실제 금융망(FIX 4.2 대외계(FEP) 공동망 등)과 실기관 연계는 법·규정·계약·보안 요구사항이 매우 높고 개인 프로젝트로 직접 붙는 것은 현실적으로 어렵다. 따라서 본 프로젝트는 **대외계/FEP를 ‘시뮬레이터’로 구현**하고, “전문 변환·세션/재전송·에러코드 매핑” 같은 **구조/패턴을 학습**하는 데 목적을 둔다. citeturn9view0turn0search2
...
관련 기관·표준은 문맥상 다음을 기준으로 참조한다: entity["organization","KRX","korea exchange"], entity["organization","FIX Trading Community","fix protocol standards body"], entity["organization","Internet Engineering Task Force","standards body"], entity["organization","OWASP","appsec foundation"], entity["organization","W3C","web standards consortium"]. citeturn9view0turn4search7turn6search3turn7search2turn3search8
...
│  - Business Flow: account 조회, 주문 플로우, 알림         │
...
│  - 주문 체결(FIX 4.2 NewOrderSingle), 잔고/한도 검증              │
...
│  - FIX Simulator(ExecutionReport) 응답 시뮬레이션               │
└────────────────────────────────────────────────────────────┘
```
...
- 계좌 조회
  - 내 계좌 목록/잔액/최근 주문내역(요약)
- 주문
  - 매수/매도 주문(FIX 4.2 NewOrderSingle) + 체결(ExecutionReport, FEP Simulator 경유)
  - 2단계 확인(입력→검증→재인증→실행→결과)
- 알림
  - 주문 체결/거부/보안 이벤트를 WebSocket 또는 SSE로 전달(학습용)

이 구성은 “금융기관 서비스가 내부 처리 후 외부 공동망 연계로 이어질 수 있음”이라는 큰 흐름을 ‘주문 유즈케이스’ 하나로 집약해 보여주기 위함이다. citeturn9view0turn4search2
...
- `POST /api/v1/orders/prepare` : 주문 입력 검증 + "주문 세션(orderSessionId)" 발급(Redis)
- `POST /api/v1/orders/{orderSessionId}/confirm` : OTP/간편비번 검증
- `POST /api/v1/orders/{orderSessionId}/execute` : 최종 실행(FEP FIX 4.2 NewOrderSingle 전송)
- `GET /api/v1/orders/{orderId}` : 결과 조회(재조회/재시도 대비)

이 단계적 플로우는 "주문은 한 번의 POST로 끝나지 않고, 검증/인증/확정 같은 단계 상태가 존재한다"는 증권 도메인 특성을 학습하기 위한 의도다(설계 선택).
...
- 계좌조회(민감 데이터는 마스킹)
- 주문: 준비/인증/실행/체결(에러코드 포함), FEP Simulator 경유 여부, 주문번호(cl_ord_id)
- 권한 상승/관리자 행위
...
계정계는 “정합성·원장·주문 이력”이 중심이며, 채널계에서 들어온 요청을 실제로 반영하는 역할을 맡는다. citeturn4search11turn9view0  
...
원장/주문 모델은 개념적으로 아래 엔티티를 권장한다.

- `customer` : 고객(식별자, 이름 등 최소)
- `account` : 계좌(계좌상태, 상품유형, 통화, 한도)
- `balance` : 잔액(가용/주문가능/보류금) — 단순화 가능
- `ledger_entry` : 원장 분개(차변/대변, 금액, 기준시각, 참조주문)
- `order` : 주문(상태 머신: ORDER_SUBMITTED → EXECUTING → ORDER_FILLED/ORDER_REJECTED)
...
주문 체결에서 최소로 지켜야 할 동시성 규칙(구현 규칙):

- 주문 계좌의 잔액 처리는 **하나의 DB 트랜잭션**에서 완결
- 같은 계좌에 대한 동시 주문 처리 경합은 **locking read(예: `SELECT ... FOR UPDATE`)**로 직렬화
...
- 주문 체결(FIX 4.2):
  - 1. "주문 세션 생성(ORDER_SUBMITTED)"을 먼저 기록
  - 2. FEP Simulator 호출 → FIX NewOrderSingle 전송(타임아웃/재시도/서킷브레이커 적용)
  - 3. ExecutionReport(ExecType=FILL/REJECTED) 수신 후 "ORDER_FILLED/ORDER_REJECTED"로 상태 전이 + 잔고 스냅샷 반영

이 구조는 "FEP가 실패해도 내부 원장이 꼬이지 않게" 설계하는 학습 목적이다.
...
하지만 주문 실행은 보통 `POST`(비멱등 가능)로 표현되므로, 앱 레벨에서 **중복 실행 방지 키**가 필요하다(설계 선택).

권장 구현:

- 채널계가 `clientRequestId(UUID)`를 생성해 계정계로 전달
- 계정계 `order_records` 테이블에 `(clientRequestId, fromAccountId)` 유니크 제약
- 동일 키 재호출 시 "이미 처리된 orderId/상태"를 반환

이 처리는 “재시도에 안전한 금융 API”라는 포인트를 만들기 좋다.

## 대외계 FEP 시뮬레이터 및 연계 규약

대외계(FEP)는 외부 거래소/공동망과의 연계를 전담하는 영역이다. 본 프로젝트에서는 FIX 4.2 NewOrderSingle 전송 → ExecutionReport 수신 흐름을 "KRX/KOSDAQ FEP Simulator"로 구현한다.
...
- 계정계: "동시 주문 처리 경합" 테스트(멀티스레드/멀티요청) → 최종 잔액/원장 일관성 검증
- 채널계: 주문 플로우 상태머신(prepare→confirm→execute) 시나리오 테스트
...
- 채널계: p95 응답시간, 로그인 실패율, 주문 실패율(거부율), 외부 호출 timeout 비율
...
- 로그인/로그아웃, 계좌목록, 계좌상세(주문내역), 주문(단계형), 알림(실시간)
- "주문 진행 상태(prepare/confirm/execute)"를 시각적으로 보여주면 백엔드 상태머신 설계가 전달된다(설계 선택).
