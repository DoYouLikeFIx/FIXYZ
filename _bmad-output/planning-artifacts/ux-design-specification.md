...
| Login MFA → MFA Recovery           | Route navigation      | `/mfa-recovery`      | 분실/초기화된 인증기 복구 진입점   |
| MFA Recovery → MFA Rebind          | Route navigation      | `/mfa-recovery/rebind` | 새 authenticator 등록 단계 분리 |
| MFA Rebind Success → Login         | Forced redirect       | `/login`             | 로컬 인증상태 제거 후 재로그인 강제 |
| Session expiry (any screen)        | Forced redirect       | `/login`             | FR-05, NFR-S7                      |

**Order Flow implementation note:** React `useState` 또는 `useReducer`로 step(`A`\|`B`\|`C`) 관리. Step A는 주문 초안 입력 및 세션 생성, Step B는 authorization guidance / conditional step-up, Step C는 confirmation / execute를 의미한다. `react-router-dom` route 변경 없음. Browser DevTools Network tab에서 modal 전환 시 Document 타입 요청 0건 — NFR-UX1 검증 수단.
...
| ------------------------ | ------------------------------- | ----------------------------------------------------------- | ------------ |
| Login password submit    | `POST /api/v1/auth/login`                       | 버튼 비활성화 + spinner                                     | p95 500ms    |
| Login MFA verify submit  | `POST /api/v1/auth/otp/verify` 또는 최초 등록 시 `POST /api/v1/members/me/totp/confirm` | 버튼 비활성화 + spinner, 인증 완료 전까지 보호 화면 진입 차단 | p95 500ms    |
| MFA recovery bootstrap   | `POST /api/v1/members/me/totp/rebind` 또는 `POST /api/v1/auth/mfa-recovery/rebind` | 버튼 비활성화 + spinner, proof/currentPassword 실패 시 결정적 가이드 | p95 500ms |
| MFA rebind confirm submit | `POST /api/v1/auth/mfa-recovery/rebind/confirm` | 버튼 비활성화 + spinner, 성공 시 즉시 로그인 화면으로 전환 | p95 500ms |
...
| Order Step-Up Verify (조건부) | `POST /api/v1/orders/sessions/{orderSessionId}/otp/verify` | 이유 문구 유지 + OTP 6자리 입력 완료 시 즉각 피드백          | p95 500ms    |
| Order Execute            | `POST /api/v1/orders/sessions/{orderSessionId}/execute` | "주문 체결 중입니다" + 버튼 비활성화                             | p95 1,000ms  |
...
| Order Flow A      | 종목/수량 입력 → 주문 세션 생성 + authorization decision 수신 | 보유수량 부족 → "보유수량이 부족합니다. 가용수량: X주 / 주문수량: Y주"                        |
| Order Flow B      | 조건부 추가 인증 이유 표시 + OTP 검증 성공 시 Step C | 주문 세션 만료 → "주문 세션이 만료되었습니다. 주문 정보를 다시 입력해 주세요." + [처음으로] |
| Order Flow C      | 승인된 주문 확인 후 실행 → ClOrdID 표시 + 포지션 업데이트 | FEP 장애 → Gateway CB OPEN(RC=9098) → "대외 연결 회선이 일시 차단 중입니다."                 |
...
FIX의 핵심 경험은 주문 플로우 완주다. 주문 modal(A→B→C)은 FIX가 주장하는 모든 백엔드 결정을 단 하나의 연속된 사용자 행동으로 증명한다: 포지션 수량 검증 → authorization decision / conditional step-up → 원자적 포지션 기록 → 실시간 체결 알림 도착. 주문 플로우가 매끄럽게 완주되는 순간, FIX의 포트폴리오 가치가 전달된다.
...
1. **Conditional OTP auto-submit:** `challengeRequired=true`인 경우에만 Step B에서 6자리 숫자 입력 완성 시 수동 "확인" 버튼 없이 자동 검증 진행 — 한국 모바일 뱅킹 표준 UX 패턴과 동일.

2. **Post-order notification:** 주문 체결 modal의 ClOrdID + FILLED 표시와 동시 또는 직전에 Notification Feed에 알림 항목 생성 (SSE 실시간 전달).

3. **Zero-config startup:** `docker compose up` → 브라우저 → 비밀번호 단계 로그인 → 로그인 MFA 완료. README/seed helper가 같은 인증 경로를 안내해야 한다.
...
3. **Demo-path optimization:** 로그인부터 주문 체결까지 11 actions 이내, 3분 이내. 최소 데모 경로: 이메일 입력 → 비밀번호 입력 → 로그인(3) → 로그인 MFA 6자리 입력(4) → 매수/매도 클릭(5) → 종목코드/수량 입력 → 다음(8) → 필요 시 주문 OTP 6자리 auto-submit 또는 auto-authorized 배너 확인(9) → 주문 실행/결과 확인(10).
...
| 비밀번호 단계 + 로그인 MFA 완료         | 안도, 신뢰 시작   | 단계적 인증 응답, HttpOnly 쿠키, 보호 화면 진입 허용 |
...
- **Retry CTA (토스/카카오뱅크/Stripe):** 실패 케이스별 명확한 재시도 경로 제공:
  - 보유수량 부족 → "수량 수정" (Step A 유지, 입력값 보존)
  - OTP 오류 → "OTP 재입력" (Step B 재렌더)
...
| `login-email`                 | Login 화면                   | Step 7 Round 1 |
...
| `order-input-qty`             | Order-A — 수량 입력       | Step 7 Round 1 |
...
| 2    | `demo@fix.com` / `demo1234` 입력             | Login             | `login-email`, `login-password`              |
| 3    | 로그인 버튼 클릭 → 로그인 MFA 화면 진입      | Login             | `login-submit`                               |
| 4    | 현재 TOTP 6자리 입력 → `/portfolio` redirect | Login MFA         | `login-mfa-input`                            |
| 5    | PortfolioCard 클릭 → `/portfolio/:id`       | Portfolio List    | `account-card-110234567890`                  |
| 6    | "매도" 버튼 클릭 → Order modal 오픈        | PortfolioDetail   | `account-detail-order-btn`                   |
| 7    | 종목 선택 + 수량 입력 (005930, 100주)        | Order-A           | `order-input-symbol`, `order-input-qty`      |
| 8    | "다음" 클릭 → authorization decision 수신    | Order-A           | `order-stepper-step-b`                       |
| 9    | 필요 시 OTP 6자리 입력 → 자동 submit          | Order-B           | `otp-input`                                  |
| 10   | Summary Card + trace ID 확인, SSE Toast 수신 | Order-Complete    | `order-summary-card`, `order-trace-id`       |
...
1. **≤3분 완주:** 비밀번호 단계 로그인 → 로그인 MFA 완료 → 포지션 선택 → 주문 A→B→C → 체결 확인까지 10 steps, 3분 이내
2. **Zero-config 시작:** `docker compose up` → seed data 자동 투입 → `demo@fix.com`/`demo1234` 비밀번호 단계 후 로그인 MFA까지 정상 완료
3. **OTP 마찰 없음:** 6자리 입력 완료 → 버튼 없이 자동 submit
4. **실시간 체결 알림 도착:** Order C 완료 → SSE event → UI Toast 2초 내 — DevTools stream 확인 가능
5. **에러도 성공:** 보유수량 부족 → 한국어 에러 메시지 → Retry CTA → Step A 자연스러운 복귀
...
- `login-email` input: `demo@fix.com` (의도적 no pre-fill — 면접관이 직접 입력하는 과정이 시연)
- `login-password` input: `demo1234`
- `login-submit` 버튼 → POST `/api/v1/auth/login` → `Set-Cookie: sessionId` (HttpOnly) → `/portfolio` redirect
- **실패:** 잘못된 자격증명 → `data-testid="error-message"` + "이메일 또는 비밀번호가 올바르지 않습니다"
- **PrivateRoute 패턴:** `/portfolio`, `/portfolio/:symbol`는 인증 guard 적용
  ...
  ```
- **`useAuth` hook:** 아래 예시는 canonical target contract 기준이다. 현재 Story 0.7 scaffold는 아직 `/api/v1/auth/session` parity endpoint를 제공하지 않으므로, Epic 12 문서가 이 target contract와 current scaffold divergence를 별도로 추적한다. HttpOnly 쿠키는 JS 직접 읽기 불가 → 앱 시작 시 `GET /api/v1/auth/session` 호출로 인증 상태 초기화
  ```ts
  function useAuth() {
...
Position quantity and valuation use the same canonical account response for MVP:

- 포지션 수량은 계좌 API가 제공한다.
- 현재가/평가금액은 같은 account response의 `marketPrice`, `quoteAsOf`, `quoteSourceMode`를 기준으로 계산한다.
- 별도 FE 전용 quote snapshot endpoint를 새로 가정하지 않는다.
- `portfolio-card-005930` 예시: 삼성전자(005930), 500주, 현재가/평가금액은 고정 상수가 아니라 response snapshot 기반 렌더다.

각 valuation surface는 formatted `quoteAsOf`와 raw `quoteSourceMode` 배지(`LIVE`/`DELAYED`/`REPLAY`)를 함께 표시한다. Web은 `PortfolioPage` summary, mobile은 `AuthenticatedHomeScreen` summary를 canonical target으로 삼는다.

`MARKET` Step A prepare가 `VALIDATION-003 (STALE_QUOTE)`로 거절되면 클라이언트는 Step A에 머물면서 stale-data guidance를 표시하고 Step B/C로 진행하지 않는다.
...
- `useReducer` 초기화: `{ step: 'A', symbol: '', qty: 0, orderSessionId: null, challengeRequired: null, authorizationReason: null, otpValue: '' }`
- `order-stepper-step-a` active

**Step A — 주문 초안 입력 + 세션 생성:**
...
│                             │
│ 수량                        │
│ [__________] 주            │  ← order-input-qty
│                             │
│              [다음 →]       │  ← disabled if invalid
└─────────────────────────────┘
```

- 종목코드/수량 validation: 공백, 포맷 불일치, 가용수량 초과 시 인라인 에러 + "다음" 버튼 disabled 또는 제출 거부
- "다음" 클릭 → `POST /api/v1/orders/sessions` 호출 (`X-ClOrdID` 포함)
- 성공 시 create response의 `orderSessionId`, `status`, `challengeRequired`, `authorizationReason`, `expiresAt`를 상태에 저장
- `challengeRequired=true`면 `dispatch({ type: 'NEXT' })` → `step: 'B'`, `order-stepper-step-b` active
- `challengeRequired=false && status==='AUTHED'`면 Step B를 건너뛰고 `step: 'C'`, `order-stepper-step-c` active

**Step B — authorization guidance + conditional step-up:**

- `authorizationReason`을 사용자 설명 문구와 함께 표시
- `challengeRequired=true`인 경우에만 `OTPInput` (`otp-input`, `otp-input-{0~5}`) 활성화
- 6자리 완성 → `aria-busy="true"` → `POST /api/v1/orders/sessions/{orderSessionId}/otp/verify` 자동 호출
- OTP 오류: 6칸 clear + `otp-input-0` 포커스 복귀 + `data-testid="error-message"`
- OTP 성공 또는 상태 재조회 결과 `status==='AUTHED'` 확인 시 `dispatch({ type: 'NEXT' })` → `step: 'C'`
- `dispatch({ type: 'PREV' })` → Step A 복귀 가능하되 draft와 `authorizationReason` 요약은 유지

> **Channel-specific continuity UX note:** web과 mobile은 같은 authorization contract를 소비하지만 continuity 판단의 강도는 다르다. Mobile에서는 같은 앱/같은 계정/같은 trusted device continuity가 유지되는 동안 network change 또는 background/resume alone 때문에 Step B를 강제하지 않는다.

**Step C — confirmation / execute:**

- 주문 요약 카드에 종목, 수량, `authorizationReason` 표시
- auto-authorized 세션은 "현재 신뢰 세션으로 승인됨" 배너 표시
- 남은 세션 시간은 평상시 상시 노출하지 않음. `remainingSeconds <= 60`일 때만 하단 비차단 warning bar에서 `세션 연장` CTA를 노출
- `주문 실행` 버튼 클릭 → `POST /api/v1/orders/sessions/{orderSessionId}/execute`
- 제출 중에는 버튼 `disabled` + `aria-busy="true"`

**Completion:**

- POST `/api/v1/orders/sessions/{orderSessionId}/execute` 성공 응답 → `dispatch({ type: 'COMPLETE', payload: result })` **즉시** → `OrderSummaryCard` 렌더
...
| --------------- | -------------------------------------------------- | ---------------------------------- | ---------------------- |
| 보유수량 부족   | Step A 인라인 에러 + "수량 수정" CTA               | `error-message`, `order-retry-btn` | 유지 (Step A)          |
| OTP 오류        | 6칸 clear + 포커스 복귀 + 에러 메시지              | `error-message`                    | 유지 (Step B)          |
...
| 세션 만료 (404) | blocking expired-session modal + restart CTA + stale `orderSessionId` 폐기 | `order-session-expired-modal`, `order-retry-btn` | 유지 → Step A 복귀     |
| 네트워크 오류   | form/session context 유지 + 재시도 안내            | `error-message`                    | 유지 → 사용자 dismiss  |
...
type OrderAction =
  ...
  | { type: "SESSION_CREATED"; payload: CreateSessionResult }
  | { type: "SET_OTP"; payload: string }
  | { type: "COMPLETE"; payload: OrderResult }
  | { type: "ERROR"; payload: string };
```

> **`PREV` vs `RESET`:** `PREV`는 step을 하나씩 되돌림 (C→B, B→A). `RESET`은 ERROR state 또는 만료 상태에서 호출 — symbol, qty를 제외한 transient session state(`orderSessionId`, `challengeRequired`, `authorizationReason`, `expiresAt`, `otpValue`)를 정리하고 Step A 복귀. SSE `EventSource`는 `COMPLETE` 또는 ERROR 시 반드시 `EventSource.close()` cleanup.

#### Resume / Restoration Rules

- Web refresh 또는 mobile background/resume 시 `orderSessionId`가 있으면 먼저 `GET /api/v1/orders/sessions/{orderSessionId}` 재조회
- `challengeRequired=true` + `status=PENDING_NEW`면 Step B와 남은 세션 문맥을 복원
- `status=AUTHED`면 Step C를 복원하고 auto-authorized 여부 배너를 다시 렌더
- `status`가 terminal이면 COMPLETE 또는 ERROR 요약 상태를 복원
- `404 ORD-008`이면 stale session을 폐기하고 Step A restart CTA로 fail-close
- Current MVP semantics: FE and MOB both follow the backend authorization decision without extra client-owned continuity weighting. A future mobile hardening lane may treat network change/background-resume as softer signals and add trusted-device/app continuity plus local transaction confirmation, but that is not part of the active MVP UX contract.

#### Order Session Expiry / Extend UX

- 주문 세션 countdown은 Step A/B/C 어디에서도 상시 노출하지 않는다
- `remainingSeconds <= 60`이 되면 `order-session-warning` 하단 sticky warning bar를 노출한다
- warning bar는 비차단이어야 하며 현재 draft, OTP 입력, confirmation context를 유지한다
- warning bar의 primary CTA는 `order-session-extend`이며 `POST /api/v1/orders/sessions/{orderSessionId}/extend`를 호출한다
- `order-session-extend`는 새 auto-auth 결정을 만드는 버튼이 아니라 active order session continuity CTA다. 따라서 사용자가 만료 전에 누르면 recent MFA freshness와 무관하게 연장 시도를 허용하고, draft / authorization summary / OTP 입력 문맥을 그대로 유지한다
- Step B에서 키패드가 열려 있으면 compact warning variant를 사용하되 CTA는 동일하게 유지한다
- 연장 성공 시 새 `expiresAt`를 저장하고 warning bar를 숨긴다
- 실제 만료 시점 또는 만료 후 다음 상호작용에서 `order-session-expired-modal` blocking dialog를 띄우고, primary CTA는 `주문 다시 시작`으로 stale session을 폐기한 뒤 Step A로 복귀시킨다
...
| "OTP는 어떻게 검증하나요?"   | Swagger `POST /api/v1/orders/sessions/{orderSessionId}/otp/verify` 엔드포인트 설명 | `nav-actuator-link` → Swagger       |
| "보유수량 초과를 시도해보면?" | Step A에서 600주 입력 → 인라인 에러 즉시         | `error-message`                     |
...
[TC-01] Happy path 주문 체결 완주
  ...
  When: Order A(100주 제출)→B(필요 시 OTP 6자리)→C(주문 실행) 완주
...
[TC-02] 보유수량 부족
  Given: Step A에서 600주 입력
  Then: error-message role="alert" 즉시 표시
        order-retry-btn 표시
        Step A 유지 (order-stepper-step-a active)

[TC-03] OTP 오류
  Given: Step B에서 잘못된 OTP 6자리 입력
  Then: 6칸 clear
        otp-input-0 포커스 복귀
        error-message 표시
...
- `useSessionExpiry.ts` — 인증 세션용 SSE EventSource 연결, `session-expiry` 이벤트 수신, 재인증 Toast 트리거
- `useOrderSessionWindow.ts` — 주문 세션 `expiresAt` 기반 warning bar / expired modal 상태 계산
...
[TC-11] 주문 세션 연장 경고
  Given: order session `expiresAt` 기준 남은 시간이 60초 이하
  Then: `order-session-warning` sticky warning bar 표시
        `order-session-extend` 클릭 시 POST `/api/v1/orders/sessions/{orderSessionId}/extend` 전송
        최근 로그인 MFA가 trusted window 밖이어도 활성 세션이면 연장 시도 허용
        성공 후 warning bar 숨김
```
...
> **Canonical target flow note**: The journey/API interactions in this section are target-contract UX flows. The current Story 0.7 scaffold is still partial and does not provide endpoint parity for every route shown here; Epic 12 documentation separately inventories the current scaffold and edge alias divergences.

### 핵심 Journey 목록

| #   | Journey            | 엔트리                         | 성공 정의                         | 연결 NFR               |
| --- | ------------------ | ------------------------------ | --------------------------------- | ---------------------- |
| J1  | Authentication     | `/login`                       | 비밀번호 단계 + MFA 완료 후 세션 발급 (SESSION) + PortfolioList 렌더링 | NFR-02, NFR-03         |
...
flowchart TD
    ...
    E -->|200 OK + nextAction=VERIFY_TOTP| F["로그인 MFA 화면 진입\n(login-mfa-input)"]
    F --> G["현재 TOTP 입력"]
    G --> H{POST /api/v1/auth/otp/verify}
    H -->|200 OK + Set-Cookie| I["SESSION HttpOnly Cookie 자동설정\nmember 정보 useAuthStore 저장"]
    I --> J["/portfolio 리다이렉트"]
    J --> K([PortfolioList 화면])
    E -->|200 OK + nextAction=ENROLL_TOTP| L["TOTP 등록 화면 진입\nQR + manualEntryKey 노출"]
    L --> M["POST /api/v1/members/me/totp/enroll\nbootstrap 수신"]
    M --> N["Google Authenticator 등록 후 첫 코드 입력"]
    N --> O{POST /api/v1/members/me/totp/confirm}
    O -->|200 OK + Set-Cookie| I
    H -->|403 AUTH-021| RA["MFA recovery 안내\n'/mfa-recovery' 진입"]
    RA --> RB["recovery proof 또는 현재 비밀번호 확인"]
    RB --> RC{POST /api/v1/auth/mfa-recovery/rebind\n또는 /api/v1/members/me/totp/rebind}
    RC -->|200 OK| RD["새 authenticator bootstrap 표시\nQR + manualEntryKey"]
    RD --> RE["현재 6자리 코드 입력"]
    RE --> RF{POST /api/v1/auth/mfa-recovery/rebind/confirm}
    RF -->|200 OK| RG["로컬 인증 상태 삭제\n'/login'으로 이동"]
    RG --> B
    E -->|401 Unauthorized| P["ErrorMessage (role=alert, break-keep)\n'이메일 또는 비밀번호가 올바르지 않습니다'"]
    P --> C
    H -->|401/410/429| Q["MFA 오류 메시지\n재입력 또는 재로그인 유도"]
    Q --> F
    RC -->|401/409/429| RH["복구/재바인드 오류 메시지\n재시도 또는 비밀번호 재설정 안내"]
    RH --> RA
    O -->|401/410| R["등록 확인 오류 메시지\n재시도 또는 재로그인 유도"]
    R --> L
    E -->|Network Error| S["ErrorMessage: '네트워크 오류'"]
    S --> D

    K --> T{세션 상태}
    T -->|TTL 300초 이하 직전| U["SSE session-expiry 이벤트 수신\nToast '세션이 곧 만료됩니다'"]
    U --> V{사용자 응답}
    V -->|"계속 사용" 클릭| VA["GET /api/v1/auth/session\nSpring Session TTL 30분 재설정"]
    VA --> T
    V -->|응답없음 / 만료| W["세션 만료 Toast 표시\n다음 요청 시 401 → /login"]
    W --> B

    K --> LO["로그아웃 버튼 클릭\n(nav-logout-btn)"]
    LO --> X["POST /api/v1/auth/logout\n서버 세션 즉시 무효화"]
    X --> B
```
...
- `AUTH-021` 수신 시 FE/MOB는 같은 의미의 복구 진입 가이드와 다음 액션을 보여야 하며, 기존 `forgot-password`와 혼동하지 않는다

### J1B — Password Recovery Challenge Flow

```mermaid
flowchart TD
    A["/forgot-password 또는 native forgot-password"] --> B["이메일 입력"]
    B --> C{POST /api/v1/auth/password/forgot}
    C -->|202 Accepted| D["동일 accepted 안내 표시\nknown/unknown parity 유지"]
    D --> E{"challenge bootstrap capability available?"}
    E -->|예: 현재 request context가 challenge-capable| F["보안 확인 준비 CTA"]
    E -->|아니오: legacy lane 유지| Z["기존 forgot/reset lane 유지"]
    F --> G{POST /api/v1/auth/password/forgot/challenge}
    G -->|200 proof-of-work bundle| H["challengeType=proof-of-work\ncontractVersion + challengeId + issued/expires + solver bundle 수신"]
    H --> H1{"bundle-trust 검증"}
    H1 -->|unknown-version / kind-mismatch / malformed-payload / mixed-shape / clock-skew / validity-untrusted| H2["active challenge clear\nrefresh CTA + canonical reason 기록"]
    H1 -->|trusted| I["로컬 solve 시작\n진행 상태 표시"]
    I --> J{사용자 cancel / expiry / refresh / background}
    J -->|cancel| K["solve 중단 + refresh CTA"]
    J -->|expiry or refresh| L["기존 challenge 폐기\n최신 challenge만 유지"]
    J -->|background/resume| M["유효성 재검증 후\n재개 또는 refresh CTA"]
    I --> N{POST /api/v1/auth/password/forgot\nsame email + challengeToken + challengeAnswer}
    N -->|202 Accepted| O["재설정 안내 유지\nredirect 의도 보존"]
    N -->|401 AUTH-022| P["잘못되었거나 만료된 proof 안내\nfresh challenge refresh CTA"]
    N -->|409 AUTH-024| Q["이미 소비된 proof 안내\nstale state clear + refresh CTA"]
    N -->|503 AUTH-023| R["bootstrap unavailable 안내\nretry later CTA"]
    N -->|503 AUTH-025| T["verify unavailable 안내\nstale challenge clear + restart CTA"]
    G -->|403 raw CSRF| S["CSRF 재조회 1회 + 동일 payload 재시도 1회"]
    N -->|403 raw CSRF| S
```

- Current canonical rollout is `proof-of-work` first. Future `captcha` adapters may reuse the same challenge branch only after the contract is extended through the discriminated `challengePayload.kind` contract.
- FE and MOB must preserve the originally submitted email string unchanged between challenge bootstrap and challenged forgot submit; normalization remains server-side.
- FE and MOB must keep the existing one-refresh and one-retry CSRF rule on challenge bootstrap and challenged forgot submit.
- Web and Mobile must render the same semantic guidance for `AUTH-022`, `AUTH-023`, `AUTH-024`, and `AUTH-025` even if their visuals differ.
- Redirect intent from a protected route must remain preserved across challenge bootstrap, challenged forgot submit, reset success, and return-to-login completion.
- `challengeMayBeRequired` is a challenge-bootstrap capability signal for the current request context, not proof that the current forgot submit is already challenge-gated.
- FE and MOB track the active challenge by opaque `challengeId`; a newly issued distinct `challengeId` replaces prior transient challenge state, and UI must never present more than one active challenge.
- Clients use `challengeIssuedAtEpochMs` / `challengeExpiresAtEpochMs` as the authoritative challenge window; after background or resume they may continue only if validity can still be guaranteed. Clients apply a `5s` expiry safety margin before starting or resuming solve work. Entering that safety window, or losing validity confidence after background/resume, is canonical fail-closed reason `validity-untrusted`. If estimated clock skew between local receipt time and `challengeIssuedAtEpochMs` exceeds `30s`, canonical fail-closed reason is `clock-skew`; in both cases clients discard the challenge and refresh.
- Unknown `challengeContractVersion`, discriminator mismatch (`challengeType` vs `challengePayload.kind`), or malformed proof-of-work payload is a fail-closed client state: clear the active challenge, show refresh guidance, and do not attempt a local solve. The only valid legacy v1 fallback shape is `challengeToken + challengeType + challengeTtlSeconds` with all v2-only fields absent.
- Canonical client fail-closed bundle rejection reasons are `unknown-version`, `kind-mismatch`, `malformed-payload`, `mixed-shape`, `clock-skew`, and `validity-untrusted` from the shared auth-error contract.
- “Newer” challenge replacement means greater `challengeIssuedAtEpochMs`; if distinct challenge bundles ever arrive out of order, the client keeps only the bundle with the greatest authoritative issue timestamp and discards the older one.
- Within the same recovery-flow scope, a newly issued challenge replaces the previous one completely; UI must never present more than one active challenge.
- If rollout flag remains off, or the request is outside the explicit QA/canary challenge-capable cohort, the backend returns the exact legacy v1 challenge bootstrap contract and upgraded FE and MOB clients preserve the existing Story `1.8` / Story `1.9` legacy recovery behavior instead of treating the response as an unknown-version failure.
- If `AUTH-023` includes `Retry-After`, FE and MOB surface the countdown and suppress immediate retry until it elapses. If it does not, they show generic retry-later guidance with no automatic retry.
...
```mermaid
flowchart TD
    A["로그인 MFA 또는 설정 화면"] --> B{복구 진입 유형}
    B -->|세션 있음| C["현재 비밀번호 확인"]
    B -->|세션 없음| D["비밀번호 재설정 성공 후\n휘발성 recoveryProof 보유"]
    C --> E{POST /api/v1/members/me/totp/rebind}
    D --> F{POST /api/v1/auth/mfa-recovery/rebind}
    E -->|200| G["rebind bootstrap 렌더링\n(mfa-recovery-qr, mfa-recovery-manual-key)"]
    F -->|200| G
    G --> H["새 authenticator에 현재 6자리 코드 입력"]
    H --> I{POST /api/v1/auth/mfa-recovery/rebind/confirm}
    I -->|200| J["auth store/session state clear"]
    J --> K["/login 복귀 + 성공 안내\n새 비밀번호 + 새 TOTP 필요"]
    E -->|401/409/429| L["복구 실패 안내 + 재시도 CTA"]
    F -->|401/409/429| L
    I -->|401/409/429| L
```

- Web canonical routes: `/mfa-recovery`, `/mfa-recovery/rebind`
- Mobile canonical auth route keys: `mfaRecovery`, `mfaRecoveryRebind`
- Unknown MFA error는 correlation/support context를 유지한 채 fallback guidance로 수렴해야 한다
...
flowchart TD
    ...
    subgraph stepA ["Step A — 주문 초안 입력 + 세션 생성"]
        B --> C["order-input-symbol 입력"]
        C --> D["order-input-qty 입력"]
        D --> E{종목/수량 유효성}
        E -->|유효하지 않음| F["inline border-destructive 에러"]
        F --> D
        E -->|유효| G["다음 버튼 활성화"]
        G --> H["POST /api/v1/orders/sessions\n→ orderSessionId + status + challengeRequired + authorizationReason"]
    end

    H --> I{challengeRequired?}
    I -->|true| J["Stepper Step-B\n(order-stepper-step-b)"]
    I -->|false, status=AUTHED| P["Stepper Step-C 활성화\n신뢰 세션 승인 배너"]

    subgraph stepB ["Step B — authorization guidance + conditional step-up"]
        J --> K["authorizationReason 설명 문구 표시"]
        K --> L["OTP 입력\notp-input-0 ~ otp-input-5\naria-label='OTP n번째 자리'\n(마지막 칸 완성 → 자동 verify)"]
        L --> M{POST /otp/verify}
        M -->|성공| P
        M -->|실패 1-2회| N["ErrorMessage: 'OTP 오류'\n재입력 (otp-input-0 자동 포커스)"]
        N --> L
        M -->|실패 3회| O["OTP 잠금 + restart CTA"]
    end

    P --> Q["주문 요약\n(종목, 수량, authorization reason,\nClOrdID 미리보기)"]
    Q --> R["주문하기 버튼"]
    R --> S["disabled + Loader2 animate-spin"]

    S --> T{주문 API}
    T -->|200 OK| U["OrderSummary\norder-summary-card\norder-clordid\norder-trace-id\nposition-qty-after"]
    U --> V([완료 화면])
    V --> W["포트폴리오로 돌아가기\n(btn-outline)"]
    W --> X([PortfolioDetail — 포지션 업데이트])

    T -->|503 + X-CB-State: OPEN| Y["cb-fallback-msg (role=alert)\norder-retry-btn\n(10초 카운트다운 후 재활성화)"]
    Y --> Z{재시도}
    Z -->|HALF-OPEN 성공| U
    Z -->|HALF-OPEN 실패| Y
    Z -->|취소| AA([Dialog 닫힘])

    T -->|404 ORD-008| AB["stale session clear\nStep A restart CTA"]
    AB --> B
    T -->|Network Timeout| AC["ErrorMessage: '요청 시간 초과'\n재시도 또는 취소"]
    AC --> Z
```

**최적화:**

- `challengeRequired=true`일 때만 OTP 마지막 칸 완성 → 즉시 `/otp/verify` 호출
- 클라이언트 blur 검증 + 서버 submit 검증 이중 구조
- auto-authorized 세션은 Step B를 건너뛰고 Step C로 직행
...
[TC-11] Order 전체 Happy Path
  Given: demo 계정 로그인 MFA 완료, 포지션(005930 삼성전자 500주) 선택
...
**Purpose:** 인증 세션 SSE `session-expiry` 이벤트 수신 시 재인증 또는 세션 연장 안내 Toast 표시  
...
**Hook:** `useSessionExpiry()` — 인증 세션 SSE `session-expiry` 이벤트 청취  
**Accessibility:** `role="alert"`, `aria-live="assertive"`

---

#### `OrderSessionExpiryWarning` (Order 내부 컴포넌트)

**Purpose:** 주문 세션 `expiresAt`가 60초 이내로 접근했을 때 비차단 sticky warning bar를 표시하고 연장 CTA를 제공  
**Anatomy:** 하단 sticky warning bar + 설명 문구 + `세션 연장` 버튼  
**States:** hidden / warning / critical / extending / expired  
**Behavior note:** `세션 연장`은 최근 MFA freshness를 다시 심사하는 버튼이 아니라 기존 active order session continuity를 유지하는 CTA다. 만료 전이라면 draft와 authorization context를 보존한 채 `expiresAt`만 새 3600초 창으로 갱신한다.
**Props:**

```ts
interface OrderSessionExpiryWarningProps {
  remainingSeconds: number;
  compact?: boolean;
  isExtending: boolean;
  onExtend: () => void; // POST /api/v1/orders/sessions/{orderSessionId}/extend
}
```

**Hook:** `useOrderSessionWindow()` — `expiresAt` 기반 남은 시간 계산, `remainingSeconds <= 60`에서 warning 상태 전환, 만료 시 expired modal 표시  
**Accessibility:** warning bar는 비차단 상태 유지, expired modal은 `role="dialog"` + `aria-modal="true"`
...
src/
  components/
    ...
    order/
      ...
      OTPInput.tsx
      OrderSummary.tsx
      OrderSessionExpiryWarning.tsx
    common/
      ErrorMessage.tsx
  hooks/
    useSessionExpiry.ts
    useOrderSessionWindow.ts
    useOrder.ts
  lib/
...
| **P1** | `OrderHistoryTable`, `ErrorMessage`, `SessionExpiryToast`, `OrderSessionExpiryWarning` | 품질 + 완성도    |
...
src/__tests__/
  components/
    ...
    OrderSessionExpiryWarning.test.tsx
  hooks/
    useOrder.test.ts
    useSessionExpiry.test.ts
    useOrderSessionWindow.test.ts
  lib/
    formatters.test.ts
```
...
| `SessionExpiryToast` | 인증 세션 SSE 이벤트 수신, 세션 유지 CTA, aria-live      |
| `OrderSessionExpiryWarning` | 주문 세션 60초 이내 경고, extend CTA, expired modal 연계 |
...
| 경고 (인증 세션)    | Toast (Sonner)              | `SessionExpiryToast` `role="alert"`                   | "5분 후 자동 로그아웃됩니다." |
| 경고 (주문 세션)    | Sticky warning bar          | `order-session-warning`, `order-session-extend`       | "주문 세션이 곧 만료돼요" |
| 경고 (주문 세션 만료) | Blocking modal            | `order-session-expired-modal`                         | "세션이 만료됐어요. 주문을 다시 시작해 주세요." |

**에러 메시지 언어 규칙:**

- 사용자 잘못: `break-keep` 한국어 — "이메일 또는 비밀번호가 올바르지 않습니다"
