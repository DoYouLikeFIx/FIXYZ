...
3. **Demo-path optimization:** 로그인부터 주문 체결까지 11 actions 이내, 3분 이내. 최소 데모 경로: 이메일 입력 → 비밀번호 입력 → 로그인(3) → 로그인 MFA 6자리 입력(4) → 매수/매도 클릭(5) → 종목코드 입력 → 수량 입력 → 다음(8) → 주문 OTP 6자리 auto-submit(9) → FILLED 확인(10).
...
| 7    | 종목 선택 (005930 삼성전자)                  | Order-A           | `order-input-symbol`                         |
| 8    | 100 입력 + "다음" 클릭                     | Order-B           | `order-input-qty`                            |
| 9    | OTP 6자리 입력 → 자동 submit                 | Order-C           | `otp-input`                                  |
...
- 6자리 완성 → `aria-busy="true"` → POST `/api/v1/orders/sessions/{orderSessionId}/execute` **자동 호출** (버튼 클릭 불필요)
...
| "OTP는 어떻게 검증하나요?"   | Swagger `POST /api/v1/orders/sessions/{orderSessionId}/otp` 엔드포인트 설명 | `nav-actuator-link` → Swagger       |
