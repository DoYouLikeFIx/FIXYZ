...
3. **Demo-path optimization:** 로그인부터 주문 체결까지 10 actions 이내, 3분 이내. 최소 데모 경로: 이메일 입력 → 비밀번호 입력 → 로그인(3) → 매수/매도 클릭(4) → 종목코드 입력 → 수량 입력 → 다음(7) → OTP 6자리 auto-submit(8) → FILLED 확인(9).
...
2. **Zero-config 시작:** `docker compose up` → seed data 자동 투입 → `demo@fix.com`/`demo1234` 로그인 즉시 성공
