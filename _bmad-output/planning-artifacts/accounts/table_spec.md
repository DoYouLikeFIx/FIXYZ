...
- 이체 처리 시 **동시성 제어의 락 타깃(row lock)** 이 된다.
    
    → `SELECT ... FOR UPDATE`로 계좌 row를 잠그고, 중복 출금/경합을 원천 차단한다.
    
- 계좌 종료는 삭제가 아니라 **상태 + 종료시각**으로 남긴다.
    
    → 감사/추적성(은행 도메인 핵심)을 유지한다.
...
| `account_number` (UK) | 실제 계좌 조회/이체 대상 식별에 쓰이는 **업무 키**. 숫자만 저장해 인덱스 효율을 확보하고, 중복 계좌번호 생성을 DB에서 차단. |
| `bank_code` | 다중 은행/기관 코드 확장 대비. 계좌번호만으로 은행을 식별하지 못하는 경우를 대비해 분리 저장. |
| `status` (ENUM) | 계좌의 현재 업무 가능 상태를 명확히 제한(ACTIVE/FROZEN/CLOSED). “종료/동결 계좌 거래 차단” 같은 핵심 정책을 단순하게 구현 가능. |
...
| `balance` (DECIMAL) | 화면/검증/리턴을 위한 **현재 잔액 캐시**. `DECIMAL(19,4)`로 정밀도 보장. `CHECK(balance >= 0)` 제약으로 버그성 초과출금 차단. |
...
- `UK(account_number)`: 계좌번호로 직접 조회/이체 검증이 많음 + 중복 방지
...
`journal_entries`는 **거래 1건(transaction_id)** 에 대한 **전표의 헤더(요약/상태/종류)** 를 저장한다.

- ledger_entries(라인)만 있으면 “이 거래가 어떤 유형의 업무였는지/상태가 무엇인지/실패 사유가 무엇인지”를 파악하기 어렵다.
- 전표 헤더를 두면:
    - 거래 분류(당행/타행/보상/조정)
...
| `transaction_id` (UUID, UK) | 전표-원장-업무를 관통하는 **거래 식별자**. “한 transaction_id = 전표 헤더 1개”를 강제하여 정합성을 단순화. |
| `client_request_id` (UUID, UK) | 멱등의 핵심 키. 같은 요청이 재시도되어도 같은 전표를 재사용/조회 가능하게 함. |
| `original_transaction_id` (IDX) | 보상/조정 전표가 “어떤 원본 거래를 되돌리는지”를 추적하는 최소 단위 키. |
| `original_journal_entry_id` (IDX) | 원본을 더 정확히 특정하고 싶을 때 유용(원본 전표 헤더를 직접 가리킴). 감사/리포트/분쟁 대응에서 추적이 쉬워짐. (물리 FK는 파티셔닝/확장성 고려로 생략 가능) |
| `journal_type` | 전표가 어떤 성격인지 분류(당행이체/타행이체/보상/조정). “보상은 새 transaction_id로 발행”이라는 규칙을 타입으로 명확히 표현. |
| `journal_status` | 전표 처리 상태(PENDING/POSTED/FAILED). 업무 상태(transfer_records.status)와 분리해 “원장 반영” 여부를 독립적으로 판단 가능. |
...
- `IDX(original_transaction_id, created_at)`: 원본 기준으로 보상 내역 타임라인 조회
- `IDX(original_journal_entry_id, created_at)`: 원본 전표 헤더 기준의 정확한 추적

---

## 3) `transfer_records` (이체 업무 레코드/복구 타겟)

### 3.1 테이블 목적(왜 필요한가)

`transfer_records`는 “원장(ledger)만으로는 표현하기 어려운 **업무 관점 상태**”를 영속 저장하는 테이블이다.

- 원장은 “회계 사실”을 저장하지만,
    - 이체 요청이 언제 들어왔는지
    - 현재 업무 상태가 무엇인지(REQUESTED/EXECUTING/COMPLETED/FAILED/COMPENSATED)
    - 타행 FEP 호출 결과가 불확실인지
...
| `public_id` (UUID, UK) | 사용자/외부에 노출 가능한 “이체 참조 번호”. CS/로그 추적에 핵심. |
| `client_request_id` (UUID, UK) | **업무 레벨 멱등키**. 재시도/중복 호출이 와도 같은 transfer를 조회/응답할 수 있어야 함. |
| `transaction_id` (IDX) | 전표/원장과 연결되는 거래 식별자. UK로 강제하지 않고 IDX로 두어, 미래에 “한 업무에 여러 전표” 확장 여지를 남김. |
| `transfer_type` | 당행/타행 분기(특히 to_account_id 채움 여부, FEP 여부 등)를 명확하게 하기 위해 필요. |
| `status` | 업무 상태 머신의 핵심. RecoveryScheduler가 스캔/복구 판단을 할 때 주로 쓰는 컬럼. |
| `failure_reason` | 실패/불확실 상태의 분기 근거. 복구/보상 로직에서 중요. |
| `amount` (DECIMAL) | 이체 금액. 원장에도 존재하지만, 업무 조회/응답/리포트에서 자주 쓰이므로 업무 레코드에 중복 저장(denormalization)하는 게 실용적. |
| `currency_code` (CHAR 3) | 통화 코드. |
| `from_account_id` | 출금 계좌. 거래내역 조회/락 대상. |
| `to_account_id` (nullable) | SAME_BANK일 때만 내부 계좌로 연결. 타행은 NULL로 강제해 “당행/타행”의 모델을 DB에서 명확히 분리. |
| `to_account_number` | 타행 포함 수취 계좌번호(정규화). “입금내역 조회(수취 기준)” 같은 기능을 위해 저장. |
| `to_bank_code` | 타행 이체의 수취 은행 식별. |
| `fep_reference_id` | 대외계/FEP 트랜잭션 추적 키. timeout/불확실 상태에서 조사/복구에 필요. |
| `post_execution_balance` | 멱등 응답 시 “당시 결과”를 안정적으로 재현하기 위한 핵심 스냅샷(예: 출금 후 잔액). |
...
- `IDX(to_account_number, created_at)`: 수취 기준 내역 조회(타행 포함) 최적화
- 타입별 `to_account_id` 체크: “당행은 내부 FK / 타행은 계좌번호” 모델을 DB에서 보장
...
| `transaction_id` (IDX) | 업무/전표/원장을 관통하는 거래 식별자. 복구 판정(예: CREDIT 존재)에도 직접 사용. |
...
## 5) `transfer_record_diagnostics` (진단 정보, 별도 분리)

### 5.1 테이블 목적

메인 테이블(`transfer_records`)의 **Row Size를 최적화**하기 위해, 대용량 텍스트나 상세 에러 로그, 원문 전문(JSON 등)을 분리 저장한다.

### 5.2 컬럼 명세

| 컬럼 | 필요 이유 |
| --- | --- |
| `transfer_record_id` (PK, FK) | 메인 테이블과 1:1 관계. |
