...
* **Purpose:** 포지션 락(`PESSIMISTIC_WRITE` on `(account_id, symbol)`) 획득 ➔ 자산 재검증(현금/포지션/일일매도한도 + MARKET stale quote 검증) ➔ FEP 발송 API 호출 ➔ 원장(Positions/Ledger) 엔티티 반영 트랜잭션 수행.
* **Request:** 
  ```json
  {
    "accountId": "1",
...
* [API-CH-06] SELL 체결 전, DB에 저장된 `Account.dailySellLimit` 값이 초과되지 않도록 그 날 자정 이후의 해당 종목 `executions` 매도 수량을 QueryDSL로 동적 Sum(합산)하여 실시간 검증해야 합니다.
