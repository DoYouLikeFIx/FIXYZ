...
> **계좌 자동 생성 실패 시 정리 정책**: `corebank-service` 계좌 생성 호출 실패 시 `channel-service`는 회원 레코드를 즉시 삭제하는 정리 트랜잭션을 실행한다 (`@Transactional` 경계 밖 Saga 패턴). 정리 실패 시 `member.status = ACCOUNT_CREATION_FAILED`로 마킹하고 별도 배치 정리 작업에서 재처리한다. 클라이언트에는 `CORE-001 503`이 반환되며 재등록을 안내한다.
...
> **채널→계정계 내부 전달 계약(MARKET)**: `channel-service`는 Step A 승인 시 `corebank-service` 내부 요청에 `quoteSnapshotId`, `quoteAsOf`, `quoteSourceMode`, `preTradePrice`를 포함해 전달한다. `MARKET` 주문에서 이 4개 값이 누락되거나 `quoteAsOf`가 staleness 임계치를 초과하면 `VALIDATION-003 422 (STALE_QUOTE)`를 반환한다.
...
PENDING_NEW → AUTHED → EXECUTING → COMPLETED
           ↘ FAILED (OTP 3회 초과, OTP_EXCEEDED)
PENDING_NEW/AUTHED → EXPIRED (TTL 만료)
EXECUTING → FAILED (명확한 거절/검증 실패)
...
| `FEP-003` | 400 | FEP 거래 거절 (`ORDER_REJECTED`) — canonical fill 유지. 세션은 복구 경로(`REQUERYING` 또는 즉시 `ESCALATED`)로 전환되어 운영자 Replay/Requery로 최종 상태 확정 | — |
| `SYS-001` | 500 | 내부 오류 | — |
...
> **`FAILED` · `ESCALATED` 상태 응답**: `status`가 `FAILED`일 때 `failureReason`의 가능한 값과 의미는 다음과 같다. `"POSITION_LOCK"` = 잔액·포지션 부족으로 서버 측에서 거절, `"MANUAL_REJECT"` = 운영자 Admin Replay로 REJECT 처리, `"OTP_EXCEEDED"` = OTP 시도 3회 초과로 세션 강제 종료. `status`가 `ESCALATED`일 때는 `failureReason: "ESCALATED_MANUAL_REVIEW"`로 고정 반환되며, 이는 FEP 타임아웃/거절 포함 외부 동기화 불확실 상태의 수동 처리 대기를 나타낸다 (종료 코드가 아님). 클라이언트는 이 값을 사용자에게 노출하지 않고 UX 메시지 매핑에만 사용해야 한다.
...
# 주문 실패 (운영자 수동 REJECT 또는 정책 위반)
data: {"type":"ORDER_FAILED","orderSessionId":"sess-uuid-xxxx","clOrdId":"client-uuid-xxxx","symbol":"005930","side":"BUY","failureReason":"MANUAL_REJECT","timestamp":"2026-03-01T10:05:35Z"}
...
| `ORDER_FAILED` | 주문 실패 이벤트. 페이로드 `failureReason`: `"POSITION_LOCK"` \| `"MANUAL_REJECT"` \| `"OTP_EXCEEDED"` (OTP 3회 초과). FEP 타임아웃/거절은 `ESCALATED` 상태로 분류되어 수동 재처리 후 최종 이벤트가 발행된다. |
  ...
  "data": {
    "content": [
      ...
      },
      {
        ...
        "type": "ORDER_FAILED",
        "payload": { "orderSessionId": "sess-uuid-yyyy", "clOrdId": "client-uuid-yyyy", "symbol": "005930", "side": "BUY", "failureReason": "MANUAL_REJECT" },
        "isRead": false,
...
> - `ORDER_FAILED`: `{ orderSessionId, clOrdId, symbol, side, failureReason }` — `failureReason`은 `"POSITION_LOCK"` \| `"MANUAL_REJECT"` \| `"OTP_EXCEEDED"` (OTP 3회 초과). FEP 타임아웃/거절은 `ESCALATED` 상태로 분류된다.
...
```json
{
  ...
  "approvedBy": "admin-uuid-second",
  "evidenceRef": "OPS-INC-20260301-17",
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `manualDecision` | Enum | ✅ | `APPROVE` (체결 확정) \| `REJECT` (실패 확정) |
| `approvedBy` | String(UUID) | ✅ | 2차 승인자 UUID. 1차 승인자(`operatorId`)와 달라야 한다 (dual-control). |
| `evidenceRef` | String | ✅ | 근거 레퍼런스 ID (운영 티켓/거래소 공시 ID 등). |
...
> **Virtual Fill 운영 통제(필수)**:
> 1. 1차 승인자(`operatorId`) + 2차 확인자(`approvedBy`)를 감사 로그에 모두 기록한다(dual-control).
> 2. 입력 `executionPrice`는 직전 quote snapshot 대비 허용 편차(`maxVirtualFillDeviationBps`)를 초과하면 승인 불가(`VALIDATION-002 422`).
> 3. `reason`은 30자 이상 구체 사유를 강제하고, 근거 레퍼런스(운영 티켓/거래소 공시 ID)를 함께 저장한다.
...
> `approvedBy`는 요청 바디 필수이며 `operatorId`와 동일하면 `VALIDATION-004 422`를 반환한다.
...
| `VALIDATION-002` | 422 | Virtual Fill 가격 편차 초과 (`executionPrice`가 허용 편차 `maxVirtualFillDeviationBps` 범위 밖) |
| `VALIDATION-004` | 422 | 수동재처리 거버넌스 필드 위반 (`approvedBy` 누락/`operatorId`와 동일, `evidenceRef` 누락, `reason` 길이 미달) |
| `RATE-001` | 429 | Rate Limit 초과 (관리자 세션당 20 req/min). `Retry-After` 헤더 포함 |
...
| `VALIDATION-001` | 422 | 요청 필드 유효성 실패 |
| `VALIDATION-002` | 422 | Virtual Fill 가격 편차 초과 |
| `VALIDATION-003` | 422 | `STALE_QUOTE` — MARKET quote snapshot staleness 또는 내부 전달 계약 위반 |
